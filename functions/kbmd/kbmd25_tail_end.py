...
################################################################################
#   Purpose:                                                                   #
#       Provides a rational Remez expansion for kbmd25 with x close to 1.      #
################################################################################
#   Author: Ryan Maguire                                                       #
#   Date:   May 19, 2026.                                                      #
################################################################################
...
def shifted_kbmd25(x_val):
    """
        Function:
            shifted_kbmd25
        Purpose:
            Computes a shifted version of kbmd25 to allow for an expansion
            of kbmd25(x) at x = 1.
        Arguments:
            x_val (float):
                The input for the window.
        Output:
            shifted_kbmd25_x (float):
                The shifted kbmd25 function at x.
    """

    if x_val == 0:
        numer = -(2.5 ** 2) * tmpld.mpmath.pi**2
        denom = tmpld.mpmath.besseli(0, 2.5 * tmpld.mpmath.pi) - 1
        return numer / denom

    x_mpf = tmpld.mpmath.mpf(x_val)
    x_shift = x_mpf + 1 / 2
    return kbmd25(x_shift) / x_mpf
...
if __name__ == "__main__":
    (p, q, e) = tmpld.remez.rat_remez(shifted_kbmd25, 8, 6, -0.25, -1.0E-15)
