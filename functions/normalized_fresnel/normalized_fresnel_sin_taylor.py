...
################################################################################
#   Purpose:                                                                   #
#       Taylor coefficients for the normalized Fresnel sine function.          #
################################################################################
#   Author: Ryan Maguire                                                       #
#   Date:   October 9, 2024.                                                   #
################################################################################
...
def fresnel_sin_taylor(x_val, index):
    """
        Function:
            fresnel_sin_taylor
        Purpose:
            Computes the nth Term of the Taylor series for the
            normalized Fresnel sine function evaluated at x.
        Arguments:
            x_val:
    ...
    xpow = factor ** (4*index + 2)
    denom = tmpld.mpmath.mpf(4*index + 3) * tmpld.mpmath.factorial(2*index + 1)
