...
################################################################################
#   Purpose:                                                                   #
#       Computes the Remez coefficients for Fresnel sine.                      #
################################################################################
#   Author: Ryan Maguire                                                       #
...
def get_coeffs(index, deg):
    """
        ...
        Purpose:
            ...
            the Fresnel sine on the interval [1 + n/32, 1 + (n+1)/32].
        Arguments:
            index:
    ...
    def shifted_f_sin(x_val):
        """
            Computes the Fresnel sine function
            for a shifted, or translated, variable.
        """
        shift = tmpld.mpmath.mpf(1) + tmpld.mpmath.mpf(index) / 32
        x_mpf = tmpld.mpmath.mpf(x_val)
        y_val = x_mpf + shift
        return fresnel_sin(y_val)

    return tmpld.remez.remez(shifted_f_sin, deg, 0, 1/32, interactive = False)

# Precisions:
#   Single:         DEGREE =  3
#   Double:         DEGREE =  8
#   Extended:       DEGREE =  9
#   Double-Double:  DEGREE = 14
#   Quadruple:      DEGREE = 15
DEGREE = 9
REMEZ_COEFFS = []
...
tmpld.string.print_mpf_array(REMEZ_COEFFS, padding = " "*4, suffix = "L,")
